...
use caaqi_context::{DetachedNode, ScopeKind, WORLD, attach_node, collect_in_scope};
...
pub fn detached_action(action: impl FnMut() + Send + Sync + 'static) -> DetachedNode<ActionScope> {
    DetachedNode::from_entity(WORLD.with(|ctx| ctx.spawn(ActionNode(Box::new(action))).id())).into()
}
...
#[track_caller]
pub fn run_action_node(node: Entity) {
    let mut action_node = WORLD.with(|ctx| {
...
pub fn defer_action_eval(mut commands: Commands, action: impl FnMut() + Send + Sync + 'static) {
